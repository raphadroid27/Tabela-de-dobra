...
from src.utils.janelas import desabilitar_janelas, habilitar_janelas, posicionar_janela
from src.utils.utilitarios import obter_caminho_icone


class FormAutenticacao:
    """Classe do formulário de autenticação."""
    ...
    def __init__(self, root, app_principal=None, login=True):
        ...
        self.auten_form.protocol(
            "WM_DELETE_WINDOW",
            lambda: [
                habilitar_janelas(app_principal),
                self.auten_form.destroy() if self.auten_form else None,
            ],
        )
    ...
    def configurar_campo_admin(self, admin_existente):
        ...
        if not admin_existente:
            ...
            admin_checkbox = tk.Checkbutton(
                self.main_frame, variable=self.admin_var, onvalue="admin", offvalue="viewer"
            )
    ...
    def configurar_botoes(self):
        ...
        if self.login:
            ...
            tk.Button(
                self.main_frame, text="Login", command=lambda: login(self.app_principal, self)
            ...
            tk.Button(self.main_frame, text="Salvar", command=lambda: novo_usuario(self)).grid(
