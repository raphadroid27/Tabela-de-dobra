...
class FormCanal:
    """Classe do formulário de canais."""
    ...
    def criar_frame_busca(self, app_principal):
        ...
        tk.Button(frame_busca, text="Limpar", command=lambda: limpar_busca("canal", self)).grid(
    ...
    def criar_frame_edicoes(self, app_principal):
        ...
        frame_edicoes = configurar_frame_edicoes(
            self.frame, text="Novo Canal" if not app_principal.editar_canal else "Editar Canal"
        )

        tk.Label(frame_edicoes, text="Valor:", anchor="w").grid(
            row=0, column=0, padx=2, sticky="sw"
        )
        self.canal_valor_entry = tk.Entry(frame_edicoes)
        self.canal_valor_entry.grid(row=1, column=0, padx=5, sticky="ew")

        tk.Label(frame_edicoes, text="Largura:", anchor="w").grid(
            row=0, padx=2, column=1, sticky="sw"
        )
        self.canal_largura_entry = tk.Entry(frame_edicoes)
        self.canal_largura_entry.grid(row=1, column=1, padx=5, sticky="ew")

        tk.Label(frame_edicoes, text="Altura:", anchor="w").grid(
        ...
        tk.Label(frame_edicoes, text="Comprimento total:", anchor="w").grid(
        ...
        tk.Label(frame_edicoes, text="Observação:", anchor="w").grid(
            row=4, column=0, padx=2, sticky="sw"
        )
    ...
    def configurar_botoes(self, frame_edicoes, app_principal):
        ...
        if app_principal.editar_canal:
            ...
            if self.canal_lista:
                self.canal_lista.bind(
                    "<ButtonRelease-1>", lambda event: preencher_campos("canal", self)
                )
            ...
            tk.Button(
                ...
                bg="green",
            ).grid(row=1, column=2, padx=5, pady=5, sticky="ew", rowspan=5)
        else:
            if self.canal_form:
