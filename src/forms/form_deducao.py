...
class FormDeducao:
    """Classe do formulário de deduções."""
    ...
    def criar_frame_busca(self, app_principal):
        ...
        self.deducao_material_combo.bind(
            "<<ComboboxSelected>>", lambda event: buscar("dedução", self)
        )
        ...
        self.deducao_espessura_combo.bind(
            "<<ComboboxSelected>>", lambda event: buscar("dedução", self)
        )
        ...
        tk.Button(
            frame_busca, text="Limpar", width=10, command=lambda: limpar_busca("dedução", self)
    ...
    def criar_lista_deducoes(self, app_principal):
        ...
        self.deducao_lista["displaycolumns"] = (
            "Material",
            "Espessura",
            "Canal",
            "Dedução",
    ...
    def criar_frame_edicoes(self, app_principal):
        ...
        frame_edicoes = configurar_frame_edicoes(
            self.frame,
            text="Nova Dedução" if not app_principal.editar_deducao else "Editar Dedução",
        )
    ...
    def configurar_botoes(self, frame_edicoes, app_principal):
        ...
        if app_principal.editar_deducao:
            ...
            if self.deducao_lista:
                self.deducao_lista.bind(
                    "<ButtonRelease-1>", lambda event: preencher_campos("dedução", self)
                )
