            ...
            for nome_arquivo in arquivos_encontrados:
                ...
                if os.path.exists(foxit_path):
                    try:
                        ...
                        resultado_impressao += f"  ✓ Sucesso com Foxit\n"
                        sucesso = True
                ...
                if not sucesso:
                    try:
                        resultado_impressao += f"Imprimindo {nome_arquivo} com impressora padrão...\n"
                        os.startfile(caminho_completo, "print")
                        resultado_impressao += f"  ✓ Sucesso com impressora padrão\n"
                        sucesso = True
                    ...
                    for adobe_path in adobe_paths:
                        if os.path.exists(adobe_path):
                            try:
                                ...
                                subprocess.run([adobe_path, "/p", caminho_completo], check=True, timeout=30)
                                resultado_impressao += f"  ✓ Sucesso com Adobe\n"
                                sucesso = True
                                break
                            except (subprocess.TimeoutExpired, subprocess.CalledProcessError, FileNotFoundError) as e:
...
def criar_frame_arquivos(main_frame):
    ...
    tk.Label(frame_entrada_multiplos, text="Lista de arquivos (um por linha):", font=("Arial", 8, "bold")).grid(row=0, column=0, columnspan=2, sticky="w")
...
010464519"""
    ...
    tk.Label(frame_lista, text="Arquivos na lista:", font=("Arial", 8, "bold")).grid(row=0, column=0, columnspan=2, sticky="w", pady=(0, 2))
