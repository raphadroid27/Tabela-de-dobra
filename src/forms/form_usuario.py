...
from src.utils.usuarios import tornar_editor, resetar_senha, excluir_usuario
...
class FormUsuario:
    """Classe do formulário de usuários."""
    ...
    def criar_frame_busca(self, app_principal):
        ...
        self.usuario_busca_entry.bind("<KeyRelease>", lambda event: buscar("usuario", self))

        tk.Button(frame_busca, text="Limpar", command=lambda: limpar_busca("usuario", self)).grid(
    ...
    def configurar_botoes(self, app_principal):
        ...
        tk.Button(
            self.frame, text="Excluir", command=lambda: excluir_usuario(self), bg="red", width=10
...
def main(root, app_principal=None):
    ...
    try:
        ...
        return  # Se tem permissão, criar o formulário
    form = FormUsuario(root, app_principal)
