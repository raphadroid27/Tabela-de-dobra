...
def carregar_interface(app_principal=None):
    ...
    if app_principal.dobras_ui:
        ...
        app_principal.cabecalho_ui.app_principal = (
            app_principal  # Adicionar referência para todas as colunas
        )

    # Criar botões
    app_principal.frame_botoes = criar_botoes(app_principal.frame_superior, app_principal)
    columnspan = len(app_principal.valores_w)
    app_principal.frame_botoes.grid(
        row=2, column=0, sticky="ewns", ipadx=2, ipady=2, columnspan=columnspan
    )

    # Configurar grid weights para as colunas
    for i, w in enumerate(app_principal.valores_w):
        app_principal.frame_superior.columnconfigure(
            i, weight=1
        )  # Restaurar valores    for w in app_principal.valores_w:
...
def configurar_menu(app_principal):
    ...
    file_menu.add_command(
        label="Nova Dedução",
        command=lambda: form_false(form_deducao, "editar_deducao", app_principal.janela_principal),
    )

    file_menu.add_command(
        label="Novo Material",
        command=lambda: form_false(
            form_material, "editar_material", app_principal.janela_principal
        ),
    )

    file_menu.add_command(
        label="Nova Espessura",
        command=lambda: form_false(
            form_espessura, "editar_espessura", app_principal.janela_principal
        ),
    )

    file_menu.add_command(
        label="Novo Canal",
        command=lambda: form_false(form_canal, "editar_canal", app_principal.janela_principal),
    )
    ...
    edit_menu.add_command(
        label="Editar Dedução",
        command=lambda: form_true(form_deducao, "editar_deducao", app_principal.janela_principal),
    )

    edit_menu.add_command(
        label="Editar Material",
        command=lambda: form_true(form_material, "editar_material", app_principal.janela_principal),
    )

    edit_menu.add_command(
        label="Editar Espessura",
        command=lambda: form_true(
            form_espessura, "editar_espessura", app_principal.janela_principal
        ),
    )
    ...
    edit_menu.add_command(
        label="Editar Canal",
        command=lambda: form_true(form_canal, "editar_canal", app_principal.janela_principal),
    )
    ...
    opcoes_menu.add_checkbutton(
        label="No topo",
        variable=app_principal.no_topo_var,
    ...
    ferramentas_menu.add_command(
        label="Razão Raio/Espessura",
        command=lambda: abrir_form_razao_rie(app_principal.janela_principal, app_principal),
    )  # Menu Usuário
    usuario_menu = tk.Menu(menu_bar, tearoff=0)
    menu_bar.add_cascade(label="Usuário", menu=usuario_menu)
    usuario_menu.add_command(
        label="Login",
        command=lambda: abrir_form_aut(app_principal.janela_principal, app_principal, login=True),
    )

    usuario_menu.add_command(
        label="Novo Usuário",
        command=lambda: abrir_form_aut(app_principal.janela_principal, app_principal, login=False),
    )
    ...
    help_menu.add_command(
        label="Sobre",
        command=lambda: abrir_form_sobre(app_principal.janela_principal, app_principal),
    )
