...
def adicionar_deducao():
    ...
    if not espessura_obj:
        show_error("Erro", f"Espessura '{espessura_valor}' não encontrada no banco.")
        return
    if not canal_obj:
        show_error("Erro", f"Canal '{canal_valor}' não encontrado no banco.")
        return
    if not material_obj:
        show_error("Erro", f"Material '{material_nome}' não encontrado no banco.")
        return
...
def adicionar_material():
    ...
    if material_existente:
        show_error("Erro", "Material já existe no banco de dados.", parent=g.MATER_FORM)
        return
...
def excluir(tipo):
    ...
    if not selected_items:
        show_error("Erro", f"Nenhum {tipo} selecionado para exclusão.", parent=config['form'])
        return
    ...
    if tipo != 'dedução':
        if tipo == 'canal':
            deducao_objs = session.query(Deducao).filter(Deducao.canal_id == obj.id).all()
        elif tipo == 'material':
            deducao_objs = session.query(Deducao).filter(Deducao.material_id == obj.id).all()
        elif tipo == 'espessura':
            deducao_objs = session.query(Deducao).filter(Deducao.espessura_id == obj.id).all()
        else:
            deducao_objs = []
...
def item_selecionado(tipo):
    ...
    try:
        ...
        if tipo == 'dedução':
            ...
            try:
                espessura_valor = float(espessura_texto) if espessura_texto else None
            ...
            obj = session.query(config['modelo']).filter_by(valor=valor).first()
...
def buscar(tipo):
    ...
    if tipo != 'dedução' and (config.get('busca') is None or not hasattr(config['busca'], 'isVisible')):
        return
