...
def adicionar_deducao(form_ui):
    ...
    deducao_existente = (
        ...
        .filter_by(
            espessura_id=espessura_obj.id, canal_id=canal_obj.id, material_id=material_obj.id
        )
        .first()
    )
...
def adicionar_espessura(form_ui):
    ...
    if espessura_existente:
        messagebox.showerror(
            "Erro", "Espessura já existe no banco de dados.", parent=form_ui.espessura_form
        )
        return
...
def adicionar_material(form_ui):
    ...
    if not nome_material:
        messagebox.showerror(
            "Erro", "O campo Material é obrigatório.", parent=form_ui.material_form
        )
        return
    ...
    if material_existente:
        messagebox.showerror(
            "Erro", "Material já existe no banco de dados.", parent=form_ui.material_form
        )
        return
...
def excluir(tipo, form_ui, app_principal):
    ...
    if obj is None:
        messagebox.showerror(
            "Erro", f"{tipo.capitalize()} não encontrado(a).", parent=config["form"]
        )
        return
    ...
    messagebox.showinfo(
        "Sucesso", f"{tipo.capitalize()} excluído(a) com sucesso!", parent=config["form"]
    )
