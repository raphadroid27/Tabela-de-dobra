...
class WidgetStateManager:
    ...
    def capture_current_state(self):
        ...
        try:
            ...
            print(f"[STATE] Estado capturado com sucesso. Widgets no cache: {len(self.widget_cache)}")
                                ...
                                try:
                                    ...
                                    if widget.currentText() != value:
                                        ...
                                        if index >= 0:
                                            ...
                                            # Adicionar item se não existir (para comboboxes editáveis)
                                            if widget.isEditable():
                                                widget.addItem(value)
                                                widget.setCurrentText(value)
                                except Exception as e:
                                    print(f"[STATE] Falha ao restaurar combobox {widget_name}: {e}")
                                    
                                print(f"[STATE] Restaurado {widget_name}: '{value}' (combobox) -> atual: '{widget.currentText()}'")
                            elif hasattr(widget, 'setText'):
                                widget.setText(value)
                                print(f"[STATE] Restaurado {widget_name}: '{value}' (entry)")
                        except Exception as e:
                            print(f"[STATE] Erro ao restaurar {widget_name}: {e}")
