...
from src.utils.banco_dados import session, tratativa_erro, obter_configuracoes
    ...
    if usuario_obj:
        if usuario_obj.senha == "nova_senha":
            nova_senha = simpledialog.askstring(
                "Nova Senha", "Digite uma nova senha:", show="*", parent=g.AUTEN_FORM
            )
            if nova_senha:
...
def tem_permissao(tipo, role_requerida, form_ui):
    ...
    if not usuario_obj:
        messagebox.showerror(
            "Erro", "Você não tem permissão para acessar esta função.", parent=config["form"]
        )
        return False
...
def resetar_senha(form_ui):
    ...
    if not selected_item:
        tk.messagebox.showwarning(
            "Aviso", "Selecione um usuário para resetar a senha.", parent=form_ui.usuario_form
        )
        return
    ...
    if usuario_obj:
        ...
        tk.messagebox.showinfo(
            "Sucesso", "Senha resetada com sucesso.", parent=form_ui.usuario_form
        )
    else:
...
def excluir_usuario(form_ui):
    ...
    if obj.role == "admin":
        messagebox.showerror(
            "Erro", "Não é possível excluir um administrador.", parent=form_ui.usuario_form
        )
        return

    aviso = messagebox.askyesno(
        "Atenção!", "Tem certeza que deseja excluir o usuário?", parent=form_ui.usuario_form
    )
    if not aviso:
        return
...
def tornar_editor(form_ui):
    ...
    if not selected_item:
        tk.messagebox.showwarning(
            "Aviso", "Selecione um usuário para promover a editor.", parent=form_ui.usuario_form
        )
        return
    ...
    if usuario_obj:
        if usuario_obj.role == "admin":
            tk.messagebox.showerror(
                "Erro", "O usuário já é um administrador.", parent=form_ui.usuario_form
            )
            return
        if usuario_obj.role == "editor":
            tk.messagebox.showinfo(
                "Informação", "O usuário já é um editor.", parent=form_ui.usuario_form
            )
            return
        ...
        tratativa_erro()
        tk.messagebox.showinfo(
            "Sucesso", "Usuário promovido a editor com sucesso.", parent=form_ui.usuario_form
        )
        listar("usuario", form_ui)
    else:
        tk.messagebox.showerror("Erro", "Usuário não encontrado.", parent=form_ui.usuario_form)
