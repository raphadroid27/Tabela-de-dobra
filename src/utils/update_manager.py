...
def check_for_updates(current_version_str: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if Version(latest_version_str) > Version(current_version_str):
            logging.info(f"Nova versão encontrada: {latest_version_str}")
            return server_info

        return None
    except (json.JSONDecodeError, KeyError, ValueError) as e:
        logging.error(f"Erro ao ler ou processar o arquivo de versão: {e}")
        return None
...
def download_update(file_name: str) -> None:
    """
    "Baixa" (copia) o arquivo de atualização do repositório 'updates'
    para a pasta temporária 'update_temp'.

    Args:
        file_name: O nome do arquivo .zip da atualização.

    Raises:
        FileNotFoundError: Se o arquivo de origem não for encontrado.
        IOError: Se houver um erro ao copiar o arquivo.
    """
    source_path = os.path.join(UPDATES_DIR, file_name)
    if not os.path.exists(source_path):
        raise FileNotFoundError(
            f"Arquivo de atualização '{file_name}' não encontrado no diretório 'updates'.")

    os.makedirs(UPDATE_TEMP_DIR, exist_ok=True)
    destination_path = os.path.join(UPDATE_TEMP_DIR, file_name)

    try:
        shutil.copy(source_path, destination_path)
        logging.info(
            f"Arquivo de atualização '{file_name}' copiado para '{UPDATE_TEMP_DIR}'.")
    except IOError as e:
        logging.error(f"Erro ao copiar o arquivo de atualização: {e}")
        raise
...
def prepare_update_flag() -> None:
    ...
    try:
        ...
        logging.info(
            f"Sinalizador de atualização '{UPDATE_FLAG_FILE}' criado.")
    except IOError as e:
        logging.error(
            f"Não foi possível criar o sinalizador de atualização: {e}")
        raise
...
def get_update_info() -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        logging.error(
            f"Erro ao ler informações de atualização do versao.json: {e}")
        return None
