...
def posicionar_janela(form, app_principal, posicao=None):
    ...
    if posicao == "centro":
        x = app_principal.janela_principal.winfo_x() + (
            (app_principal.janela_principal.winfo_width() - largura_form) // 2
        )
        y = app_principal.janela_principal.winfo_y() + (
            (app_principal.janela_principal.winfo_height() - form.winfo_height()) // 2
        )
