...
class WidgetUsageAnalyzer:
                ...
                if 'g.' in line and not line.startswith('#'):
                    ...
                    for word in words:
                        if word.startswith('g.') and len(word) > 2:
                            ...
                            widget_name = word[2:].split('(')[0].split('[')[0].split('.')[0].split(',')[0].split(')')[0]
    ...
    def get_widgets_by_category(self) -> Dict[str, List[str]]:
        ...
        for widget in all_globals:
            if any(prefix in widget for prefix in ['MAT_COMB', 'ESP_COMB', 'CANAL_COMB', 'DED_LBL', 'RI_ENTRY', 'K_LBL', 'OFFSET_LBL', 'OBS_LBL', 'FORCA_LBL', 'COMPR_ENTRY', 'ABA_EXT_LBL', 'Z_EXT_LBL']):
                categories['cabecalho'].append(widget)
            elif any(prefix in widget for prefix in ['aba', 'medidadobra', 'metadedobra', 'blank', 'FRAME_DOBRA']):
    ...
    def generate_report(self) -> str:
        ...
        # Estatísticas gerais
        report.append(f"\nESTATÍSTICAS GERAIS:")
        report.append(f"- Total de widgets globais definidos: {len(usage_analysis['all_globals'])}")
        report.append(f"- Widgets propriamente utilizados: {len(usage_analysis['properly_used'])}")
        report.append(f"- Widgets atribuídos mas não usados: {len(usage_analysis['assigned_not_used'])}")
        report.append(f"- Widgets usados mas não atribuídos: {len(usage_analysis['used_not_assigned'])}")
        report.append(f"- Widgets nunca utilizados: {len(usage_analysis['never_used'])}")
        ...
        if usage_analysis['never_used']:
            ...
            report.append("   Estes widgets podem ser removidos do globals.py:")
            for widget in usage_analysis['never_used']:
                report.append(f"   - {widget}")
        
        if usage_analysis['assigned_not_used']:
            report.append(f"\n2. WIDGETS ATRIBUÍDOS MAS NÃO USADOS ({len(usage_analysis['assigned_not_used'])}):")
        ...
        if usage_analysis['used_not_assigned']:
            ...
            report.append("   Estes widgets são usados mas podem não estar sendo atribuídos:")
            for widget in usage_analysis['used_not_assigned']:
                report.append(f"   - {widget}")
        
        # Sugestões de melhoria
        report.append(f"\nSUGESTÕES DE MELHORIA:")
        report.append("1. Considere criar widgets sob demanda ao invés de globalmente")
        report.append("2. Agrupe widgets relacionados em classes ou estruturas")
        report.append("3. Use factory patterns para criação de formulários similares")
        ...
        for file_path, stats in file_stats.items():
            if stats['assignments'] or stats['usages']:
                ...
                if stats['assignments']:
                    report.append(f"  Atribuições: {', '.join(sorted(stats['assignments']))}")
                if stats['usages']:
                    report.append(f"  Usos: {', '.join(sorted(stats['usages']))}")
