...
from PySide6.QtWidgets import QComboBox, QLineEdit, QLabel, QTreeWidget, QDialog
...
def create_deducao_espessura_combo():
    ...
    from src.models.models import Espessura
    from src.utils.banco_dados import session
...
WidgetFactory.register_creator('MAT_DENS_ENTRY', create_material_densidade_entry)
...
class LazyWidgetProxy:
    ...
    def __getattr__(self, name):
        ...
        if self._widget is None:
            raise AttributeError(f"Widget {self.widget_name} não pôde ser criado")
...
class OptimizedWidgetManager:
    ...
    def get_widget(self, widget_name: str, create_if_missing: bool = True) -> Any:
        ...
        self.access_count[widget_name] = self.access_count.get(widget_name, 0) + 1
        ...
        if widget is None and create_if_missing:
            ...
            if widget is not None:
                self.creation_count[widget_name] = self.creation_count.get(widget_name, 0) + 1
    ...
    def print_usage_report(self):
        ...
        sorted_access = sorted(stats['access_count'].items(), key=lambda x: x[1], reverse=True)
        ...
        print(f"  Criadores registrados: {factory_stats['registered_creators']}")
