...
class RegraValidacao:
    ...
    def validate(self, value: Any, context: Dict = None) -> ResultadoValidacao:
        """Executa a validação."""
        try:
            is_valid = self.validator(value, context or {})
            return ResultadoValidacao(is_valid, "" if is_valid else self.message)
...
class ValidadorCache:
    ...
    def register_rule(self, rule: RegraValidacao):
    ...
    def validate_field(
        self, field: str, value: Any, context: Dict = None
        ...
        self._validation_cache[cache_key] = {"results": results, "timestamp": time.time()}

        return results

    def validate_form(
        self, form_data: Dict[str, Any], context: Dict = None
    ) -> Dict[str, List[ResultadoValidacao]]:
        """
    ...
    def _register_default_rules(self):
        """Registra regras de validação padrão."""  # Validação de campo obrigatório
        self.register_rule(
            RegraValidacao(
                "required",
    ...
    def _validate_unique_deducao(self, context: Dict) -> bool:
        ...
        try:
            ...
            espessura = (
                session.query(Espessura).filter_by(valor=float(context["espessura_valor"])).first()
            )
            ...
            existing = (
                ...
                .filter_by(material_id=material.id, espessura_id=espessura.id, canal_id=canal.id)
                .first()
            )
...
def validate_field(field: str, value: Any, context: Dict = None) -> List[ResultadoValidacao]:
    """Função conveniente para validar campo."""
    return cached_validator.validate_field(field, value, context)


def validate_form(
    form_data: Dict[str, Any], context: Dict = None
) -> Dict[str, List[ResultadoValidacao]]:
...
def register_custom_rule(name: str, validator: Callable, message: str, priority: int = 1):
    ...
    rule = RegraValidacao(name, validator, message, priority)
