...
# Importa os caminhos necessários do módulo utilitário centralizado
...
def check_for_updates(current_version_str: str) -> Optional[Dict[str, Any]]:
    ...
    Args:
        current_version_str: A versão atual da aplicação (ex: "2.2.1").

    Returns:
        Um dicionário com informações da nova versão se houver uma,
        caso contrário None.
    """
...
def download_update(nome_arquivo: str) -> None:
    """
    Copia o arquivo de atualização do repositório 'updates' para a pasta
    temporária 'update_temp'.
    ...
    if not os.path.exists(source_path):
        raise FileNotFoundError(
            f"Arquivo de atualização '{nome_arquivo}' não encontrado em '{UPDATES_DIR}'.")
