...
def get_latest_tag():
    ...
    try:
        tags_raw = subprocess.check_output(["git", "tag", "--sort=-v:refname"], text=True).strip()
        if not tags_raw:
...
def get_commit_messages_since_last_tag():
    """Obtém as mensagens de commit desde a última tag."""
    try:
        latest_tag = get_latest_tag()
        output = subprocess.check_output(
            ["git", "log", f"{latest_tag}..HEAD", "--pretty=format:%h - %s (%an)"], 
            text=True
        ).strip()
        ...
        with open(version_info_path, "w", encoding="utf-8") as f:
            for line in lines:
                if "filevers=" in line:
                    ...
                    f.write(f"        filevers=({version_tuple}),  # Versão do arquivo ({version})\n")
                    ...
                    if "-beta" in version:
                        ...
                        version_tuple = f"{version.replace('.', ',')},0"
                    f.write(f"        prodvers=({version_tuple}),  # Versão do produto ({version})\n")
                elif "StringStruct(u'FileVersion'" in line:
                    f.write(f"                        StringStruct(u'FileVersion', u'{version}'),\n")
                elif "StringStruct(u'ProductVersion'" in line:
                    f.write(f"                        StringStruct(u'ProductVersion', u'{version}'),\n")
                else:
                    f.write(line)
...
def generate_changelog(version):
    ...
    if os.path.exists(changelog_path):
        ...
        if "# Changelog" in existing_content:
            ...
            updated_content = f"# Changelog\n\nHistórico de mudanças do aplicativo Cálculo de Dobras\n\n{new_version_content}{existing_content}"
    else:
...
def main():
    ...
    custom_version = input(f"Próxima versão será {next_version}. Pressione Enter para aceitar ou digite uma versão personalizada: ")
