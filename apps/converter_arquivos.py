"""
Aplicativo Independente para Conversão de Arquivos.

Este aplicativo implementa um formulário para conversão de arquivos, com um layout
...
Versão independente do formulário de conversão.
"""
...
# Adicionar o diretório raiz do projeto ao sys.path para permitir importações de src
...
class FileTableWidget(ManagedFileTableWidget):
    ...
    def _insert_path(self, path: str) -> None:
        """Insere um novo caminho na tabela, criando uma nova linha."""
        row = self.rowCount()
        self.insertRow(row)
    ...
    def on_files_added(self, added_paths: List[str]) -> None:  # type: ignore[override]
        """Emite sinal quando arquivos são adicionados à tabela."""
...
class FormConverterArquivos(QDialog):
    ...
    def _select_files(self):
        ...
        dialog_filter = " ".join(extensions)
        dialog_filter = f"Arquivos ({dialog_filter});;Todos os arquivos (*)"
...
def main(parent=None):
    ...
    dialog = FormConverterArquivos(parent)
    dialog.show()
    return dialog
